...
var SidebarElements = ({ slugPrefix, items, subItemLevel }) => {
  ...
  const getHref = (slugPrefix2, pathSuffix, slug) => {
    ...
    const localePrefix = currentLocale && validLocales.includes(currentLocale) ? `/${currentLocale}` : "";
  ...
  const ElementRoot = ({
    ...
    const localizedName = typeof name === "string" ? name : name[currentLocale];
    const localizedSlug = typeof slug === "string" ? slug : slug[currentLocale];
  ...
  return /* @__PURE__ */ jsx17(Box10, { className: "sidebar-component", children: items?.map((item2, index) => {
    const key = typeof item2.slug === "string" ? String(item2.slug) + String(index) : String(item2.slug[currentLocale]) + String(index);
    const slug = typeof item2.slug === "string" ? `${item2.slug}` : `${item2.slug[currentLocale]}`;
